...
ckpt_dir="${CKPT_DIR:-saves/vanilla/mmimdb}"
suffix_prefix="${SUFFIX_PREFIX:-${importance_mode}_rep_cka}"
out_dir="${OUT_DIR:-}"
...
do
    mapfile -t ckpts < <(find "$ckpt_dir" -maxdepth 1 -type f \( -name "*_rep_cka_${regularizer_weight}_*.pth" -o -name "*_rep_cka_${regularizer_weight}.pth" \) | sort)
    if (( ${#ckpts[@]} == 0 )); then
        echo "[SKIP] dataset=mmimdb method=rep-cka lambda=${regularizer_weight} no checkpoints in $ckpt_dir"
        continue
    fi

    suffix="${suffix_prefix}_${regularizer_weight}"
    echo "[RUN] dataset=mmimdb method=rep-cka lambda=${regularizer_weight} ckpts=${#ckpts[@]} split=${eval_split} suffix=${suffix}"
...
if (( evaluated == 0 )); then
    echo "[ERROR] no evaluations were run for dataset=mmimdb method=rep-cka"
    exit 1
fi
