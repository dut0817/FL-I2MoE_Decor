...
ckpt_dir="${CKPT_DIR:-saves/vanilla/enrico}"
...
do
    ...
    if (( ${#ckpts[@]} == 0 )); then
        echo "[SKIP] dataset=enrico method=rep-cka lambda=${regularizer_weight} no checkpoints in $ckpt_dir"
        continue
    fi

    suffix="${suffix_prefix}_${regularizer_weight}"
    echo "[RUN] dataset=enrico method=rep-cka lambda=${regularizer_weight} ckpts=${#ckpts[@]} split=${eval_split} suffix=${suffix}"
...
if (( evaluated == 0 )); then
    echo "[ERROR] no evaluations were run for dataset=enrico method=rep-cka"
    exit 1
fi
