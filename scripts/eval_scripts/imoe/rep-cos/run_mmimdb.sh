...
ckpt_dir="${CKPT_DIR:-saves/imoe/transformer/mmimdb/rep_cos}"
...
do
    ...
    if (( ${#ckpts[@]} == 0 )); then
        echo "[SKIP] dataset=mmimdb method=rep-cos lambda=${regularizer_weight} no checkpoints in $ckpt_dir"
        continue
    fi

    suffix="${suffix_prefix}_${regularizer_weight}"
    echo "[RUN] dataset=mmimdb method=rep-cos lambda=${regularizer_weight} ckpts=${#ckpts[@]} split=${eval_split} suffix=${suffix}"
...
if (( evaluated == 0 )); then
    echo "[ERROR] no evaluations were run for dataset=mmimdb method=rep-cos"
    exit 1
fi
