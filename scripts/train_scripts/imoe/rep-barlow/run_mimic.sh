...
rep_barlow_loss_weight_grid="${REP_BARLOW_LOSS_WEIGHT:-${BARLOW_LOSS_WEIGHT:-$lambda_grid}}"
run_seed="${RUN_SEED:-0}"
n_runs_override="${N_RUNS_OVERRIDE:-3}"
save_subdir="${SAVE_SUBDIR:-rep_barlow}"
...
for num_heads in 4
do
for rep_barlow_loss_weight in $rep_barlow_loss_weight_grid
do
CUDA_VISIBLE_DEVICES=$device python src/imoe/train_transformer.py \
    ...
    --save True \
    --use_common_ids True \
    --regularizer rep-barlow \
    --rep_barlow_loss_weight $rep_barlow_loss_weight \
