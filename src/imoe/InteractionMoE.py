...
class MLPReWeighting(nn.Module):
    ...
    def forward(self, inputs, masks=None):
        if inputs[0].dim() == 3:
            feats = []
            for i, item in enumerate(inputs):          # item: (B, T, H)
                if masks is not None and i < len(masks) and masks[i] is not None:
                    m = masks[i].to(item.device).float().unsqueeze(-1)   # (B, T, 1)
                    num = (item * m).sum(dim=1)                          # (B, H)
                    den = m.sum(dim=1).clamp_min(1.0)                    # (B, 1)
                    v = num / den
                    v = F.normalize(v, p=2, dim=1)
                    feats.append(v)                              # masked mean
                else:
                    feats.append(item.mean(dim=1))                       # (B, H)
            x = torch.cat(feats, dim=1)                                  # (B, H*num_modalities)
        else:
            x = torch.cat(inputs, dim=1)                                 # (B, H*num_modalities)
...
class InteractionExpert(nn.Module):
    ...
    def forward(self, inputs, masks=None):
        ...
        """
        return self._forward_with_replacement(inputs, masks, replace_index=None)

    def forward_with_replacement(self, inputs, masks, replace_index):
        """
        ...
        """
        return self._forward_with_replacement(inputs, masks, replace_index=replace_index)

    def _forward_with_replacement(self, inputs, masks, replace_index=None):
        """
        ...
        logits, latent = self.fusion_model(inputs, masks, return_latent=True)
        if self.fusion_sparse:
            return (logits, latent), self.fusion_model.gate_loss()
        return (logits, latent)

    def forward_multiple(self, inputs, masks=None):
        """
        ...
        if self.fusion_sparse:
            ...
            outputs.append(self.forward(inputs, masks))

        # Forward passes with each modality replaced
        for i in range(len(inputs)):
            outputs.append(self.forward_with_replacement(inputs, masks, replace_index=i))
...
class InteractionMoE(nn.Module):
    ...
    def forward(self, inputs, masks=None):
        ...
        if self.fusion_sparse:
            ...
            for expert in self.interaction_experts:
                expert_output, expert_gate_loss = expert.forward_multiple(inputs, masks)
                expert_outputs.append(expert_output)
                expert_gate_losses.append(expert_gate_loss)
        else:
            for expert in self.interaction_experts:
                expert_outputs.append(expert.forward_multiple(inputs, masks))
        ...
        for i in range(self.num_modalities):
            ...
            anchor = outputs[0][1]
            neg = outputs[i + 1][1]
            positives = [o[1] for o in (outputs[1 : i + 1] + outputs[i + 2 :])]
        ...
        synergy_anchor = synergy_output[0][1]
        synergy_negatives = torch.stack([o[1] for o in synergy_output[1:]])
        synergy_loss = self.synergy_loss(synergy_anchor, synergy_negatives)

        # One Redundacy Expert
        redundancy_output = expert_outputs[-1]
        redundancy_anchor = redundancy_output[0][1]
        redundancy_positives = torch.stack([o[1] for o in redundancy_output[1:]])
        ...
        all_logits = torch.stack([output[0][0] for output in expert_outputs], dim=1)
        all_latents = [out_list[0][1] for out_list in expert_outputs]  # list of [B, D]
        ###### MLP reweighting the experts output ######
        interaction_weights = self.reweight(inputs, masks)  # Get interaction weights
    ...
    def inference(self, inputs, masks=None):
        expert_logits = []   # collect logits only
        if self.fusion_sparse:
            for expert in self.interaction_experts:
                eo, _ = expert.forward(inputs, masks)   # eo: Tensor or (logits, latent)
                if isinstance(eo, (tuple, list)):
                    eo = eo[0]  # keep logits only
                expert_logits.append(eo)
        else:
            for expert in self.interaction_experts:
                eo = expert.forward(inputs, masks)
                if isinstance(eo, (tuple, list)):
                    eo = eo[0]
                expert_logits.append(eo)
        ...
        interaction_weights = self.reweight(inputs, masks)
        weights_transposed = interaction_weights.unsqueeze(2)
        weighted_logits = (all_logits * weights_transposed).sum(dim=1)

        # Keep backward compatibility: return expert-wise outputs as a logits list.
        return expert_logits, interaction_weights, weighted_logits
