...
from src.common.modules.common import MLP, PatchEmbeddings, Linear, TabularTokenEncoder
...
def load_and_preprocess_data_mimic(args):
    # Paths
    lab_path = "data/mimic4 Dataset/all3/lab_x"
    note_text_path = "data/mimic4 Dataset/all3/note_texts_last256tok.csv"
    code_path = "data/mimic4 Dataset/all3/code_x"
    label_df = pd.read_csv(
        "data/mimic4 Dataset/all3/labels.csv",
        index_col="subject_id",
    )
    labels = label_df["one_year_mortality"].values.astype(np.int64)
    n_labels = len(set(labels)) 

    with open("data/mimic4 Dataset/all3/PTID_splits_mimic_processed.json") as json_file:
    ...
    if "L" in args.modality or "l" in args.modality:
        path = lab_path
        arr = torch.load(path + ".pt")  # (N, F)
        if isinstance(arr, torch.Tensor):
            arr = arr.numpy()
        # Optional: apply a scaler here if needed.
        # scaler = MinMaxScaler(feature_range=(-1, 1))
        # arr = scaler.fit_transform(arr)

        new_idx = np.arange(arr.shape[0])
        filtered_idx = new_idx[new_idx != -1]  # effectively all rows (0..N-1)
        ...
        # Use TabularTokenEncoder so each lab feature is treated as its own token.
        encoder_dict["lab"] = TabularTokenEncoder(
    ...
    if "N" in args.modality or "n" in args.modality:
        note_text_df = pd.read_csv(
            note_text_path,
            index_col="subject_id",
        )
        # Align to label_df.index ordering.
        texts = note_text_df.reindex(label_df.index)["text"].fillna("").tolist()

        data_dict["note"] = np.array(texts, dtype=object)

        # Mark observed=True only for subjects with non-empty note text.
        has_note = np.array([t.strip() != "" for t in texts], dtype=bool)
        observed_idx_arr[has_note, 1] = True

        # Include only subjects with notes in common_idx_list.
        common_idx_list.append(set(np.where(has_note)[0]))

        # Update modality combination.
        for idx, flag in enumerate(has_note):
            if flag:
                update_modality_combinations(idx, "N")

        print("Loaded note texts, using BERTTextEncoder.")

        encoder_dict["note"] = BERTTextEncoder(
        ...
        input_dims["note"] = None  # token sequence: no fixed "feature count" dimension
    ...
    if "C" in args.modality or "c" in args.modality:
        ...
        if isinstance(arr, torch.Tensor):
            arr = arr.numpy()
        # Optional: scaling can be applied here.
        # scaler = MinMaxScaler(feature_range=(-1, 1))
        # arr = scaler.fit_transform(arr)
        ...
        encoder_dict["code"] = TabularTokenEncoder(
            num_features=arr.shape[1],
            out_dim=args.hidden_dim,
        ).to(args.device)
    ...
    def all_modalities_missing(idx):
        for modality, arr in data_dict.items():
            if modality in ["modality_comb", "note"]:
                continue
            # For tabular data, preprocessing uses -2 for missing; checking the first feature is sufficient.
            if arr[idx, 0] != -2:
                return False
        return True
