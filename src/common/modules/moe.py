...
class FixedFMoE(nn.Module):
    def __init__(
        ...
        # Runtime caches for training-time analysis/regularization hooks.
        self.last_topk_indices = None
        self.last_topk_scores = None
        self.last_tot_expert = int(self.num_expert * self.world_size)
        # Optional training-time capture for regularizers that need pre-mix expert outputs.
        self.capture_topk_pre_mix = False
        self.last_topk_pre_mix = None
    ...
    def forward(self, moe_inp):
        ...
        """
        # Avoid stale tensors across steps when capture is toggled.
        self.last_topk_pre_mix = None
        ...
        self.last_topk_indices = gate_top_k_idx
        self.last_topk_scores = gate_score
        self.last_tot_expert = int(
            getattr(self.gate, "tot_expert", self.num_expert * self.world_size)
        )
        ...
        # Capture pre-mix expert outputs only when explicitly enabled.
        if self.capture_topk_pre_mix:
            self.last_topk_pre_mix = moe_outp if torch.is_tensor(moe_outp) else None
