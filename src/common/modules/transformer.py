...
class TransformerEncoderLayer(nn.Module):
    ...
    def forward(self, x, masks=None):
        ...
        # ➊ Normalize mask list: None -> ones(B, Ti)
        def _normalize_masks(masks_list, chunk_sizes, device):
            if masks_list is None:
                return [torch.ones((B, cs), device=device) for cs in chunk_sizes]
            out = []
            for m, cs in zip(masks_list, chunk_sizes):
                if m is None:
                    out.append(torch.ones((B, cs), device=device))
                else:
                    # Accept both float and bool; assume float uses 0/1 semantics.
                    out.append(m.to(device).float())
            return out

        if self.self_attn:
            cat = torch.cat(x, dim=1)                  # (B, sumT, H)
            device = cat.device
            norm_cat = self.norm1(cat)

            norm_masks = _normalize_masks(masks, chunk_size, device)  # list of (B, Ti)
            attn_key_mask = torch.cat(norm_masks, dim=1)              # (B, sumT), 1=valid/0=pad

            # ➋ Standard residual: y = cat + Dropout(Attn(LN(cat)))
            attn_out = self.attn(norm_cat, norm_cat, attn_mask=attn_key_mask)  # (B, sumT, H)
            cat = cat + self.dropout1(attn_out)

            # Split and apply MLP blocks.
            x = list(torch.split(cat, chunk_size, dim=1))  # ensure list type
            for i in range(len(chunk_size)):
                # ➌ Per token sequence: LN -> (MoE/MLP) -> Dropout -> Residual
                x[i] = x[i] + self.dropout2(self.mlp(self.norm2(x[i])))
            return x

        else:
            # (Optional) keep cross-attention path mask-safe as well.
            device = x[0].device
            norm_masks = _normalize_masks(masks, chunk_size, device)   # list of (B, Ti)
            cat_mask = torch.cat(norm_masks, dim=1)                    # (B, sumT)

            for i in range(len(chunk_size)):
                # Query is x[i]; key/value is concatenation including itself (adjust as needed).
                other = torch.cat([x[i]] + [x[j] for j in range(len(chunk_size)) if j != i], dim=1)
                # Prepare residual.
                res = x[i]
                # Applying LN to both query and key/value is common; here we keep it simple.
                q_norm = self.norm1(x[i])
                kv_norm = self.norm1(other)
                out_i = self.attn(q_norm, kv_norm, attn_mask=cat_mask)  # (B, Ti, H)
                x[i] = res + self.dropout1(out_i)
                x[i] = x[i] + self.dropout2(self.mlp(self.norm2(x[i])))
            return x
...
class Attention(nn.Module):
    def __init__(self, dim, num_heads=8, qkv_bias=False, attn_drop=0.0, proj_drop=0.0):
        ...
        self.capture_attn = True               # whether to store per-layer attention maps
        self.require_attn_grad = False         # gradient path flag for attn×grad rollout
        self.last_attn = None
    ...
    def forward(self, x, kv, attn_mask=None):
        ...
        if attn_mask is not None:
            # attn_mask: (B, Nk) with 1(valid)/0(pad)
            # broadcast to (B,1,1,Nk)
            key_mask = attn_mask.unsqueeze(1).unsqueeze(1)  # (B,1,1,Nk)
            attn = attn.masked_fill(key_mask == 0, float('-inf'))

        attn = attn.softmax(dim=-1)
        
        if self.capture_attn:
            self.last_attn = attn if self.require_attn_grad else attn.detach()
            if self.require_attn_grad and attn.requires_grad:
