...
class TabularTokenEncoder(nn.Module):
    """
    Encoder that converts a (B, F) tabular vector into a (B, T, H) token sequence.
    Here T = F (one token per feature).

    x: (B, F)
      - Example: if one row in lab_x.pt has F lab features, you get F tokens.
    Returns:
      tokens: (B, F, H)
      mask:   (B, F) (currently all ones; missing features can be marked as 0 later)
    """

    def __init__(self, num_features: int, out_dim: int, use_bias: bool = True):
        super().__init__()
        self.num_features = num_features
        self.out_dim = out_dim

        # H-dimensional embedding per feature.
        self.feature_emb = nn.Parameter(
            torch.randn(num_features, out_dim) * 0.02
        )
        if use_bias:
            self.bias = nn.Parameter(torch.zeros(num_features, out_dim))
        else:
    ...
    def forward(self, x: torch.Tensor):
        ...
        x: (B, F)
        """
        assert x.dim() == 2, f"TabularTokenEncoder expects (B,F), got {x.shape}"
        B, F = x.shape
        assert F == self.num_features, f"num_features mismatch: {F} vs {self.num_features}"

        # (B, F, 1) * (F, H) -> (B, F, H)
        x_exp = x.unsqueeze(-1)                # (B, F, 1)
        emb = self.feature_emb.unsqueeze(0)    # (1, F, H)
        tokens = x_exp * emb                   # (B, F, H)

        if self.bias is not None:
            tokens = tokens + self.bias.unsqueeze(0)  # (1, F, H) broadcast

        # For now, treat all features as valid tokens.
        mask = torch.ones(B, F, device=x.device, dtype=torch.float32)
