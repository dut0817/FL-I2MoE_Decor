...
# -*- coding: utf-8 -*-
"""
Unified GShardGate masking evaluation for 3 datasets:
...
def get_args():
    ...
    p.add_argument("--ckpt", type=str, nargs="+", required=True, help="GShardGate checkpoint path(s)")
    ...
    p.add_argument("--fusion_sparse", type=str2bool, default=True)
    p.add_argument("--gate", type=str, default="GShardGate")
...
def _extract_logits(forward_out):
    if torch.is_tensor(forward_out):
        return forward_out
    if isinstance(forward_out, (tuple, list)):
        two_d = [x for x in forward_out if torch.is_tensor(x) and x.dim() == 2 and x.size(0) > 0]
        if two_d:
            return two_d[-1]
    raise RuntimeError("Failed to extract logits from model output.")


def forward_logits(model, feats, masks):
    out = model(feats, masks=masks)
...
def attn_importance_per_modality(model, feats, masks, modal_names):
    was_training = model.training
    model.eval()

    _clear_last_attn(model)
    _set_attn_capture_flags(model, capture=True, require_grad=False)
    with torch.no_grad():
        _ = forward_logits(model, feats, masks)

    attn_list = collect_attns_from_transformer(model)
    lengths = [f.size(1) for f in feats]
    imp_global = attention_rollout(attn_list, head_fusion="mean", add_residual=True)
    imps = split_importance_by_modalities(imp_global, lengths)
    imps = normalize_importances_per_modality(imps, masks, modal_names)

    _set_attn_capture_flags(model, capture=False, require_grad=False)
    if was_training:
        model.train(True)
    return imps


def grad_attn_importance_per_modality(model, feats, masks, modal_names, task_type, target):
    was_training = model.training
    model.eval()
    model.zero_grad(set_to_none=True)

    _clear_last_attn(model)
    _set_attn_capture_flags(model, capture=True, require_grad=True)

    logits = forward_logits(model, feats, masks)
    score = build_logit_score(logits, target, task_type)
    score.backward()
...
def ig_importance_per_modality(model, feats, masks, modal_names, task_type, target, steps=16):
    was_training = model.training
    model.eval()
    ...
    for s in range(1, steps + 1):
        ...
        model.zero_grad(set_to_none=True)
        logits_alpha = forward_logits(model, x_alpha_list, masks)
...
@torch.no_grad()
def compute_base_metrics(data_loader, encoder_dict, model, device, ns, task_type, return_logits=False):
...
def run_full_test_drop_curve(eval_loader, encoder_dict, model, device, ns, args, task_type, Ks, modal_means):
    csa_largest = True
    ...
    base = compute_base_metrics(eval_loader, encoder_dict, model, device, ns, task_type)
    ...
    for it, batch in enumerate(eval_loader):
        ...
        bsz = feats[0].size(0)
        with torch.no_grad():
            logits_orig = forward_logits(model, feats, masks)
        ...
        if args.importance_mode == "attn":
            imps = attn_importance_per_modality(model, feats, masks, modal_names)
        elif args.importance_mode == "attnXgrad":
            imps = grad_attn_importance_per_modality(
                model, feats, masks, modal_names, task_type, eval_target
            )
        elif args.importance_mode == "ig":
            imps = ig_importance_per_modality(
                model, feats, masks, modal_names, task_type, eval_target, steps=args.ig_steps
            )
        else:
        ...
        for K in Ks:
            ...
            with torch.no_grad():
                logits_comp = forward_logits(model, feats_comp, masks_comp)
                logits_suff = forward_logits(model, feats_suff, masks_suff)
            ...
            if args.random_repeats > 0:
                ...
                for _r in range(args.random_repeats):
                    ...
                    with torch.no_grad():
                        logits_rnd = forward_logits(model, feats_rnd, masks_rnd)
                        logits_rnd_keep = forward_logits(model, feats_rnd_keep, masks_rnd_keep)
    ...
    df["comprehensiveness_prob_gap"] = (
        df["comprehensiveness_prob_mean_identified"]
        - df["comprehensiveness_prob_mean_random"]
    )
    df["sufficiency_prob_gap"] = (
        df["sufficiency_prob_mean_random"]
        - df["sufficiency_prob_mean_identified"]
    )
    df["aopc_comprehensiveness_prob_gap"] = (
        df["aopc_comprehensiveness_prob_identified"]
        - df["aopc_comprehensiveness_prob_random"]
    )
    df["aopc_sufficiency_prob_gap"] = (
        df["aopc_sufficiency_prob_random"]
        - df["aopc_sufficiency_prob_identified"]
    )
...
def load_ckpt_baseline(ckpt_path, encoder_dict, model, device, strict_enc=True, strict_model=False):
    sd = torch.load(ckpt_path, map_location="cpu")
    if "encoder_dict" not in sd or "fusion_model" not in sd:
        raise KeyError(f"[{ckpt_path}] checkpoint must contain 'encoder_dict' and 'fusion_model'")
    ...
    missing, unexpected = model.load_state_dict(sd["fusion_model"], strict=strict_model)
...
def build_dataset_context(args, device, device_id):
    ...
    return ns, encoder_dict, train_loader, valid_loader, test_loader, model, task_type
...
def main():
    ...
    ns, encoder_dict, train_loader, valid_loader, test_loader, model, task_type = build_dataset_context(
    ...
    prefix = f"{args.dataset}_gshardgate_masking_per_modality"
